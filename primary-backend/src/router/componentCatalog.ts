...
const Components: WorkflowComponentDefinition[] = [
  ...
  },
  {
    ...
    configSchema: z.object({
      provider: z.enum(["resend", "smtp", "sendgrid", "mailgun", "ses"]).default("resend"),
  ...
  // ─── GMAIL ─────────────────────────────────────────────────
  {
    ...
    name: "Gmail",
    description:
      "Send, read, and list emails using the **Gmail API**. Works with **free personal Google accounts**. " +
      "Connect your Google account via Dashboard → Integrations → Google.",
    icon: "Mail",
    category: "integration",
    tags: ["google", "gmail", "email", "send", "read", "inbox"],
    configFields: [
      {
        key: "authMode",
        label: "Authentication",
        ...
        options: [
          { label: "Connected Google account (OAuth)", value: "oauth_connection" },
          { label: "Manual — token / {{secrets.*}}", value: "manual" },
        ],
      },
      {
        key: "googleConnectionId",
      ...
      },
      {
        ...
        label: "Credentials / token",
        type: "password",
        placeholder: "{{secrets.GMAIL_TOKEN}}",
        showWhen: { field: "authMode", value: "manual" },
      },
      {
        key: "operation",
      ...
      },
      {
        ...
        showWhen: { field: "operation", value: "send_email" },
      },
      {
        key: "cc",
        label: "CC",
      ...
      },
      {
        ...
        showWhen: { field: "operation", value: "list_messages" },
      },
      {
        key: "maxResults",
        label: "Max Results",
      ...
      },
      {
        ...
        type: "text",
        required: true,
        placeholder: "e.g. 18f1a2b3c4d5e6f7",
        description: "The Gmail message ID to read (from list_messages output)",
        showWhen: { field: "operation", value: "read_message" },
      },
    ],
    configSchema: z.object({
      authMode: z.enum(["oauth_connection", "manual"]).default("oauth_connection"),
      googleConnectionId: z.string().optional(),
      credentialsSecret: z.string().optional(),
      operation: z.enum(["send_email", "list_messages", "read_message"]).default("send_email"),
    ...
    configFields: [
      {
        ...
        description: "Which field from the previous step's output to check. For example, 'payload.urgency' to check the urgency value returned by an AI agent.",
      },
      {
        key: "operator",
        label: "Operator",
  ...
  },
  {
    ...
    configFields: [
      { key: "valuePath", label: "Route based on", type: "text", required: true, placeholder: "payload.category", description: "Which field from the previous step's output to route on. For example, if the AI classified a ticket and returned a 'category' field, enter 'payload.category' to route based on that value." },
      { key: "cases", label: "Branches", type: "json", required: true, rows: 5, defaultValue: [{ value: "typeA", label: "Type A" }, { value: "typeB", label: "Type B" }], description: "The possible values and their branch names. Each entry has a 'value' to match and a 'label' to display." },
      { key: "defaultBranch", label: "Default Branch", type: "text", placeholder: "other", description: "Which branch to take when none of the above values match" },
    ],
    configSchema: z.object({
...
registerAliases("gmail",
  "google_mail", "GMAIL", "tool.gmail", "google-gmail", "gmail-send",
);
