...
// Execute a workflow
router.post("/run/:workflowId", authMiddleware, async (req: AuthRequest, res) => {
  try {
...
async function simulateExecution(runId: string, nodes: any[], edges: any[]) {
  ...
  if (!triggerNode) {
    await prisma.workflowRun.update({
      where: { id: runId },
      data: { status: "FAILED", error: "No trigger node found", completedAt: new Date() },
    });
    return;
  }
...
function generateNodeOutput(node: any): any {
  ...
  return outputs[node.nodeType] || { result: "completed", nodeType: node.nodeType };
}
...
function getReasoningSummary(node: any): string | null {
  ...
  return summaries[node.nodeType] || "Agent processed input and generated structured output.";
}
...
router.get("/runs/:workflowId", authMiddleware, async (req: AuthRequest, res) => {
  try {
