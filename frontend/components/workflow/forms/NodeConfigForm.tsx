...
import { getConfigFields, type ConfigField } from "../config/nodeTypes";
        ...
        return (
          <div key={field.key} className="space-y-1.5">
            <label className="flex items-center justify-between">
              <span className="text-xs font-medium flex items-center space-x-1.5" style={{ color: "var(--text-secondary)" }}>
            ...
            {field.type === "password" || field.type === "api-key" && (
              <div className="relative">
                ...
                />
                {!isEmpty && (
                  <Lock className="w-3 h-3 absolute right-2.5 top-1/2 -translate-y-1/2" style={{ color: "var(--text-muted)" }} />
                )}
              </div>
            )}
            ...
            {field.type === "boolean" && (
              ...
              >
                <div className={cn("w-2 h-2 rounded-full", value ? "bg-emerald-500" : "bg-gray-300")} />
            ...
            {field.description && (
              <p className="text-[10px] mt-1 flex items-start space-x-1" style={{ color: "var(--text-muted)" }}>
  ...
  return (
    <div className="relative">
      <div
        ...
        style={{
          background: "var(--input-bg)",
          border: value.length > 0 ? "1px solid var(--border-medium)" : "1px solid var(--border-subtle)",
          color: "var(--text-primary)"
        }}
      >
      ...
      {isOpen && (
        <div className="absolute z-10 w-full mt-1 rounded-lg border shadow-xl overflow-hidden max-h-48 overflow-y-auto"
            ...
            {filteredOptions.map((option) => {
              ...
              return (
                ...
                >
                  <span>{option.label}</span>
                  {isSelected && (
                    <div className="w-4 h-4 rounded flex items-center justify-center" style={{ background: "rgba(16,185,129,0.1)" }}>
