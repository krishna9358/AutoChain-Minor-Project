...
export const Feature = ({
  ...
  const variantClasses = {
    default: "group",
    highlight:
      "bg-accent px-4 py-3 rounded-xl border border-primary-100 shadow-sm hover:shadow-glow hover:scale-105",
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        {variant === "highlight" && (
          <div className="absolute inset-0 bg-primary/5 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out pointer-events-none" />
        )}
      </div>
    </motion.div>
  );
...
export const FeatureCard = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl bg-primary opacity-0 group-hover:opacity-10 transition-opacity duration-300 -z-10 blur-sm" />

      <div className="relative z-10">
        <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 shadow-glow">
          {icon}
        </div>
