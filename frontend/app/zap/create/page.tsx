...
import { Zap, Plus, X, Server, LayoutTemplate, Send, ArrowRight, Wallet, Mail, GripVertical } from "lucide-react";
    ...
    return (
        <div
            className={cn(
                ...
                data.selected ? "border-blue-500 shadow-blue-500/20" : "border-slate-200",
        ...
        >
            ...
            >
                {isTrigger ? <Zap className="w-6 h-6" /> : <LayoutTemplate className="w-6 h-6" />}
            </div>

            <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-slate-800 text-base truncate">
                    {data.config?.name || (isTrigger ? "Select Trigger" : "Select Action")}
                </h3>
...
function WorkflowEditor() {
    ...
    const onDrop = useCallback(
        (event: React.DragEvent) => {
            ...
            setNodes((nds) => [...nds, newNode].map(n => ({ ...n, data: { ...n.data, selected: n.id === newNodeId } })));
    ...
    const handleUpdateNode = (nodeId: string, config: AppItem | null, metadata: Record<string, unknown>) => {
        setNodes((nds) =>
            nds.map((n) => {
                if (n.id === nodeId) {
                    return {
    ...
    const validateAndPublish = async () => {
        ...
        if (actionsPipeline.length === 0 && Array.from(new Set(nodes.map(n => n.data.type))).includes("action")) {
            alert("You have actions on canvas but they are not connected to the trigger.");
            return;
        }
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <span className="text-sm font-semibold text-slate-700 group-hover:text-amber-600">Empty Trigger</span>
                            </div>
                            ...
                            {availableTriggers.map((t) => (
                                ...
                                >
                                    <GripVertical className="w-4 h-4 text-slate-300 mr-2" />
                                    <img src={t.image} className="w-8 h-8 rounded object-cover mr-3 bg-slate-50 border border-slate-100" />
                                    <span className="text-sm font-semibold text-slate-700 group-hover:text-amber-600">{t.name}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        {/* Actions Category */}
                        <div>
                            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 text-blue-500">Actions</h3>
                            ...
                            >
                                ...
                                <span className="text-sm font-semibold text-slate-700 group-hover:text-blue-600">Empty Action</span>
                            </div>
                            ...
                            {availableActions.map((a) => (
                                ...
                                >
                                    <GripVertical className="w-4 h-4 text-slate-300 mr-2" />
                                    <img src={a.image} className="w-8 h-8 rounded object-cover mr-3 bg-slate-50 border border-slate-100" />
                                    <span className="text-sm font-semibold text-slate-700 group-hover:text-blue-600">{a.name}</span>
                                </div>
                            ))}
                        </div>
                ...
                <div className="flex-1 relative h-full" ref={reactFlowWrapper}>
                    ...
                    {nodes.length === 0 && (
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center pointer-events-none text-slate-400 flex flex-col items-center">
                            ...
                            <h2 className="text-xl font-bold text-slate-600 mb-2">Build your workflow</h2>
                            <p>Drag any trigger or action from the left sidebar onto this canvas.</p>
                        </div>
                    )}
                </div>
                        ...
                        >
                            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 sticky top-0 z-10">
                                <div>
                                    <h2 className="text-lg font-bold text-slate-800">
                                        {activeNode.data.type === "trigger" ? "Configure Trigger" : "Configure Action"}
                                    </h2>
                                    <p className="text-sm text-slate-500">Edit integration settings</p>
                                </div>
                                <button
                                    onClick={onPaneClick}
                            ...
                            <div className="p-6 flex-1 bg-white">
                                <NodeConfigurator
                                    ...
                                    onUpdate={(config, metadata) => handleUpdateNode(activeNode.id, config, metadata)}
                                />
                            </div>
                        </motion.div>
                    )}
...
function NodeConfigurator({
    ...
    const [localMeta, setLocalMeta] = useState<Record<string, any>>(node.data.metadata || {});
        ...
        return (
            <div className="space-y-4">
                ...
                {items.map((item) => (
                    ...
                    >
                        <img src={item.image} alt={item.name} className="w-10 h-10 rounded-lg mr-4 object-cover border border-slate-100 bg-slate-50 p-1" />
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center space-x-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <img src={selectedItem?.image} alt={selectedItem?.name} className="w-12 h-12 rounded-lg object-cover border border-slate-200 bg-white p-1" />
                <div className="flex-1">
                    <h3 className="font-semibold text-slate-800">{selectedItem?.name}</h3>
                    <p className="text-xs text-blue-600 font-medium">{isTrigger ? "Source Event" : "Output Action"}</p>
                </div>
                <button
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                ...
                <div className="p-5 space-y-4">
                    {currentConfigId === "email" && (
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 flex items-center"><Mail className="w-4 h-4 mr-1 text-slate-400" /> Recipient Email</label>
                                <input
                                    type="email"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 flex items-center"><LayoutTemplate className="w-4 h-4 mr-1 text-slate-400" /> Message Body</label>
                                <textarea
                    ...
                    {currentConfigId === "send-sol" && (
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 flex items-center"><Wallet className="w-4 h-4 mr-1 text-slate-400" /> Wallet Address</label>
                                <input
                                    type="text"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-700 flex items-center"><Zap className="w-4 h-4 mr-1 text-amber-400" /> SOL Amount</label>
                                <input
                                    type="number"
                                    className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                                    placeholder="0.01"
                    ...
                    {currentConfigId === "webhook" && (
                        <div className="text-sm text-slate-600 space-y-2 p-4 bg-blue-50 rounded-lg border border-blue-100">
                            <div className="flex items-center text-blue-700 font-medium mb-1"><Server className="w-4 h-4 mr-2" /> Webhook URL</div>
                            <p>A unique, secure Webhook URL will be automatically generated upon workflow publish.</p>
                            <p className="text-xs text-slate-500 mt-2">Send a POST request with JSON payload to explicitly trigger this flow.</p>
                        </div>
                    )}
