...
const services = [
  { name: 'postgres', label: 'Database', port: 5432, healthCheck: 'pg_isready' },
  { name: 'kafka', label: 'Kafka', port: 9094, healthCheck: 'kafka-topics' },
  { name: 'backend', label: 'Backend API', port: 3001, url: 'http://localhost:3001' },
  { name: 'hooks', label: 'Webhook Receiver', port: 3002, url: 'http://localhost:3002' },
  { name: 'processor', label: 'Event Processor', dependsOn: ['kafka', 'postgres'] },
  { name: 'worker', label: 'Workflow Worker', dependsOn: ['kafka', 'postgres'] },
  { name: 'frontend', label: 'Frontend', port: 3000, url: 'http://localhost:3000' },
];
...
function banner() {
  console.log('\n' + colorize('╔══════════════════════════════════════════════════════════╗', 'cyan'));
  console.log(colorize('║  ', 'cyan') + colorize('⚡ AgentFlow AI ', 'blue') + colorize('— Development Environment', 'cyan') + '               ║');
...
function showServiceUrls() {
  log('\n🌐 Service URLs:', 'cyan');
  services.filter(s => s.url).forEach(service => {
...
function cmdUp() {
  ...
  setTimeout(() => {
    ...
    showServiceUrls();
    log('\n💡 Development mode is ON — no login required!', 'yellow');
    log('💡 Source files are volume-mounted for hot reload', 'yellow');
    log('\n' + colorize('Tips:', 'bright'));
    log('  • Run ' + colorize('node scripts/dev.js logs', 'cyan') + ' to view logs');
    log('  • Run ' + colorize('node scripts/dev.js status', 'cyan') + ' to check status');
    log('  • Run ' + colorize('node scripts/dev.js down', 'cyan') + ' to stop all services');
    console.log('');
  }, 5000);
}
